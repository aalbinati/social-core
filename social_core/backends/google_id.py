"""
Sign In With Google Id authentication backend.
...
Settings:
    ...
    * `GoogleIdAuth.TOKEN_TTL_SEC` - time before JWT token expiration, seconds.
    * `SOCIAL_AUTH_GOOGLE_ID_INACTIVE_USER_LOGIN` - allow inactive users email to
                                                   login
"""
...
class GoogleIdAuth(BaseOAuth2):
    ...
    TOKEN_AUDIENCE = "https://accounts.google.com"
    ...
    def auth_params(self, *args, **kwargs):
        """
        Google requires to set `response_mode` to `form_post` if `scope`
        parameter is passed.
        """
    ...
    def get_google_jwk(self, kid=None):
        """
        Return requested Google public key or all available.
        """
    ...
    def decode_id_token(self, id_token):
        """
        Decode and validate JWT token from google and return payload including
        user data.
        """
        if not id_token:
        ...
        try:
            ...
            public_key = RSAAlgorithm.from_jwk(self.get_google_jwk(kid))
            decoded = jwt.decode(
                id_token,
                key=public_key,
